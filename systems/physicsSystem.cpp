...
extern  std::shared_ptr<TerrainSystem> terrainSystem;
...
void PhysiqueSystem::Update(float dt){
	...
	for (auto const& go : mGameObject){
		moveInPlanXZ(dt, go);
		if (gCoordinator.HaveComponent<AABB>(go)) {
			testCollision(dt, go);
		}
...
void PhysiqueSystem::testCollision(float dt, GameObject go){
	...
	if (move != glm::vec3(0.f, 0.f, 0.f)) {//pas de déplacement = pas de collision
		...
		for (auto const& go2 : mGameObject) {
			if (go2 != go) {
				if (gCoordinator.HaveComponent<AABB>(go2)) {
					...
					/*if (gCoordinator.HaveComponent<Chunk>(go2)) {
						...
						Transform& transform = gCoordinator.GetCompenent<Transform>(go);
						int min = terrainSystem->getChunkAt(terrain, aabbDeplacement.min.x, aabbDeplacement.min.z);
						int max = terrainSystem->getChunkAt(terrain, aabbDeplacement.max.x, aabbDeplacement.max.z);
						if (min == -1 || max == -1) {
							move = { 0.f, 0.f, 0.f };
						}
						else {
							if (min == max) {//on se déplacement sur un même chunk
								bool collide = false;
								if (terrainSystem->getSolInChunkAt(gCoordinator.GetCompenent<Chunk>(go2), aabbDeplacement.max.x, aabbDeplacement.max.y-1, aabbDeplacement.max.z)) {
									int nbRemonte = 1;
									while (terrainSystem->getSolInChunkAt(gCoordinator.GetCompenent<Chunk>(go2), aabbDeplacement.max.x, aabbDeplacement.max.y-nbRemonte-1, aabbDeplacement.max.z)) {
										nbRemonte++;
									}
									//move = { 0.f, 0.f, 0.f };
									move.y -= nbRemonte;
