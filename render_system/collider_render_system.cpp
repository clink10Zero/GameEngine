...
namespace lve {
    ...
    void ColliderRenderSystem::createPipeline(VkRenderPass renderPass) {
        ...
        lvePipeline = std::make_unique<LvePipeline>(
            lveDevice,
            "shaders/collider_shader.vert.spv",
            "shaders/collider_shader.frag.spv",
            pipelineConfig, true);
    }
    ...
    void ColliderRenderSystem::renderCollider(FrameInfo& frameInfo, GameObject gameObject, Transform parentTransform) {
        ...
        Mesh m{};
        m.data = createAABBModel(gameObject);
    ...
    std::unique_ptr<LveModel>  ColliderRenderSystem::createAABBModel(GameObject go)
    {
        Mesh mesh = gCoordinator.GetCompenent<Mesh>(go);
